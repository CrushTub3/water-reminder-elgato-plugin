...
reminderAction.onKeyDown(({ action, context, device, event, payload }) => {
	...
	MACTIONS[context].waterConsumed();
});
...
class ReminderAction {
    ...
    didReceiveSettings(settings) {
		...
		if(settings.hasOwnProperty('reminderInterval')) {
			...
			dirty = true;
			console.log("New Setting Reminder Interval: " + settings.reminderInterval);
        }
	...
	updatePercentage() {
		if(this.percentage >= 0)
		{
			...
			const timeDeltaSeconds = (currentTime - new Date(this.lastDrankTime)) / 1000; // Convert milliseconds to seconds
			this.percentage = Math.max(0, Math.floor(100 - (timeDeltaSeconds / (this.settings.reminderInterval * 60)) * 100));
	...
	getSVGImage(percentage) { // Percentage Goes from 0 (empty) to 100 (full)
